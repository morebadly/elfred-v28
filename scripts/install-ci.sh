...
if [[ -n "${seed_cache}" && -d "${seed_cache}" ]]; then
  ...
  if [[ "${seed_lockfile_sha256}" == "${lockfile_sha256}" || "${SITES_INSTALL_OFFLINE:-0}" == "1" ]]; then
    echo "[sites] restoring image-seeded npm cache"
    cp -a "${seed_cache}/." "${expected_cache}/"
    use_seeded_cache=1
    echo "[sites] local cache restored; npm will verify the project's locked package integrities"
  else
...
if [[ "${use_seeded_cache}" == "0" && "${SITES_INSTALL_OFFLINE:-0}" != "1" ]]; then
...
if [[ "${SITES_INSTALL_OFFLINE:-0}" == "1" ]]; then
  npm_ci_args+=(--offline)
fi
