...
export function InactiveMemories(){
 const runtime=useRuntime()!,items=runtime.snapshot!.objects.memory.filter(m=>!['deleted','rejected','superseded'].includes(text(m,'status'))&&(m.data.hidden||m.data.expires_at&&Date.parse(text(m,'expires_at'))<=Date.parse(runtime.snapshot!.server_time)));
