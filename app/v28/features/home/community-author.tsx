...
export function RecruitmentSummary({post,onOpen}:{post:Entity;onOpen:()=>void}){
 ...
 return <section className={styles.card} aria-label="共创任务预览"><header><b>共同完成</b><span>{post.data.status==='recruiting'?'招募中':'招募已结束'}</span></header>{slots.length>0?<div className={styles.stats}><span>{slots.length} 项分工</span><span>{claimed} 个参与名额已认领</span><span>{completed} 项已完成</span></div>:<div className={styles.task}><span>{text(post,'task')}</span><small>共同任务</small></div>}{slots.slice(0,3).map(slot=><div key={slot.id} className={styles.task}><span>{slot.title}</span><small>{slot.terms_changed?'约定待核对':slot.status==='completed'?'已完成':slot.status==='open'?'可参与':'已关闭'}</small></div>)}{slots.length>3&&<small>还有 {slots.length-3} 项分工</small>}<p className={styles.criteria}>验收：{text(post,'criteria')}</p><button type="button" className={styles.link} onClick={onOpen}><span>查看协作过程与成果</span><ChevronRight size={15}/></button></section>;
}
